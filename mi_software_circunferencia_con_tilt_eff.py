...
def compute_efficiency_components(s: Vec3, n: Vec3, p: Vec3, r: Vec3, cfg: Config) -> dict:
    eta_cos = clamp01(s[0]*n[0] + s[1]*n[1] + s[2]*n[2])
    dist = math.sqrt((r[0]-p[0])**2 + (r[1]-p[1])**2 + (r[2]-p[2])**2)
    eta_att = math.exp(-cfg.atm_k * dist)
    ...
    if not a.no_plot and cmds:
        try:
            ...
            def do_animation():
                ...
                # ===== TILT SUBPLOT (nuevo) =====
                ax_tilt = fig2.add_axes([0.90, 0.52, 0.08, 0.42])
                ...
                eta_txt_a = ax_tilt.text(0.50, 0.72, '', transform=ax_tilt.transAxes, va='center', fontsize=8, color='tab:green')
                eta_txt_b = ax_tilt.text(0.50, 0.22, '', transform=ax_tilt.transAxes, va='center', fontsize=8, color='tab:orange')

                # ===== EFFICIENCY SUBPLOT (gráfica evolutiva) =====
                ax_eff = fig2.add_axes([0.88, 0.06, 0.10, 0.30])
                ax_eff.set_xlim(0, len(indices))
                ax_eff.set_ylim(0.85, 1.0)
                ax_eff.set_xlabel('Frame', fontsize=8)
                ax_eff.set_ylabel('η', fontsize=8)
                ax_eff.set_title('Eficiencia', fontsize=9, color='gray')
                ax_eff.tick_params(labelsize=7)
                ax_eff.set_xticklabels([])
                ax_eff.grid(True, alpha=0.3, lw=0.5)
                # lines for efficiency evolution
                eff_line_a, = ax_eff.plot([], [], lw=1.5, color='tab:green', label='Helio')
                eff_line_b, = ax_eff.plot([], [], lw=1.5, color='tab:orange', label='CircD')
                ax_eff.legend(loc='best', fontsize=7)

                # scaling: reduce sizes (smaller vectors)
                sun_scale = 0.20 * max(1.0, math.hypot(rng_x, rng_y))
                vec_scale = sun_scale * 0.30

                effA_series: List[float] = []
                effB_series: List[float] = []
                distA_series: List[float] = []
                distB_series: List[float] = []
                eff_frame_indices: List[int] = []
                seen_frames = set()
                printed_stats = {'done': False}

                def _mean(vals):
                    return sum(vals)/len(vals) if vals else 0.0

                def _print_eff_stats():
                    if printed_stats['done']:
                        return
                    printed_stats['done'] = True
                    mean_eta_a = _mean(effA_series)
                    mean_eta_b = _mean(effB_series)
                    mean_dist_a = _mean(distA_series)
                    mean_dist_b = _mean(distB_series)
                    print(f"[eff] mean eta_total opt={mean_eta_a:.4f}  circ={mean_eta_b:.4f}")
                    print(f"[eff] mean dist opt={mean_dist_a:.3f} m  circ={mean_dist_b:.3f} m")
                ...
                def update(frame_idx):
                    ...
                    if tdir is not None:
                        ...
                        if n is not None:
                            ...
                            pA = (xk, yk, 0.0)
                            effA = compute_efficiency_components((sx, sy, sz), n, pA, rT, cfg)
                        ...
                        if tdir_b is not None:
                            ...
                            if nb is not None:
                                ...
                                try:
                                    ...
                                    pB = (xk_b, yk_b, 0.0)
                                    effB = compute_efficiency_components((sx, sy, sz), nb, pB, rT, cfg)
                                except Exception:
                                    tilt_line_b.set_data([], []); tilt_txt_b.set_text('')
                    # update time/tilt text
                    lines = [
                        f"t={t_k.isoformat(timespec='minutes')}",
                        f"r={math.hypot(xk,yk):.2f} m  tilt={tilt:.1f} deg",
                    ]
                    if effA is not None:
                        lines.append(
                    ...
                    if effA is not None:
                        eta_txt_a.set_text(f"η={effA['eta_total']:.3f}")
                    else:
                        eta_txt_a.set_text('')
                    if effB is not None:
                        eta_txt_b.set_text(f"η={effB['eta_total']:.3f}")
                    else:
                        eta_txt_b.set_text('')

                    if frame_idx not in seen_frames:
                        seen_frames.add(frame_idx)
                        eff_frame_indices.append(len(effA_series))
                        if effA is not None:
                            effA_series.append(effA["eta_total"])
                            distA_series.append(effA["dist"])
                        else:
                            effA_series.append(float('nan'))
                            distA_series.append(float('nan'))
                        if effB is not None:
                            effB_series.append(effB["eta_total"])
                            distB_series.append(effB["dist"])
                        else:
                            effB_series.append(float('nan'))
                            distB_series.append(float('nan'))

                    # update efficiency plot
                    if effA_series:
                        x_data = list(range(len(effA_series)))
                        eff_line_a.set_data(x_data, effA_series)
                    if effB_series:
                        x_data = list(range(len(effB_series)))
                        eff_line_b.set_data(x_data, effB_series)

                    return cur_pt, cur_pt_b, q_sun, q_norm, q_psi, q_norm_b, q_psi_b, line_sun, line_sun_b, line_ref_b, sun_dot, txta, tilt_line_a, tilt_line_b, tilt_txt_a, tilt_txt_b, eta_txt_a, eta_txt_b, eff_line_a, eff_line_b
