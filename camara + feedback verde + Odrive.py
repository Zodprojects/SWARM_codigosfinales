...
class Config:
    ...
    # ODrive (CANSimple) - Control de ruedas
    node_right: int = 1             # ID del nodo derecho
    node_left: int = 2              # ID del nodo izquierdo
    wheel_radius: float = 0.08      # Radio de la rueda (m)
    gear_ratio: float = 1.0         # Relación de reducción motor->rueda
    wheelbase: float = 0.30         # Distancia entre ruedas (m)
    vmax_rev_s_clip: float = 20.0   # Velocidad máxima (rev/s)
    sign_R: float = +1.0            # Signo para rueda derecha (+1 o -1)
    sign_L: float = +1.0            # Signo para rueda izquierda (+1 o -1)
    max_linear_speed: float = 0.5   # Velocidad lineal máxima (m/s)
    max_angular_speed: float = 1.0  # Velocidad angular máxima (rad/s)
...
class WheelOdometry:
    """Calcula pose (x, y, ψ) integrando velocidades de las ruedas"""
    ...
    def reset(self, x: float = 0.0, y: float = 0.0, psi: float = 0.0):
        """Reinicia la odometría a una pose conocida"""
        self.x = x
        self.y = y
        self.psi = psi
        self.last_update_time = time.time()
    ...
    def update(self, vR: float, vL: float):
        """
        Actualiza la pose usando cinemática diferencial
        
        Args:
            vR: Velocidad rueda derecha (m/s)
            vL: Velocidad rueda izquierda (m/s)
        """
        current_time = time.time()
        dt = current_time - self.last_update_time
        
        # Evitar dt muy grandes (ej. primera iteración)
        if dt > 0.5 or dt <= 0:
            self.last_update_time = current_time
            return
        
        # Cinemática diferencial
        v = (vR + vL) / 2.0  # Velocidad lineal del centro
        omega = (vR - vL) / self.cfg.wheelbase  # Velocidad angular
        
        # Integración (método de Euler)
        # En sistema ENU: x=Este, y=Norte, psi medido desde Norte en sentido horario
...
GET_ENCODER_ESTIMATES = 0x09  # Response: <f32 pos, f32 vel>
AXIS_IDLE       = 1
AXIS_CLOSEDLOOP = 8

class ODriveCanDriver:
    """Control de ODrive por CAN para movimiento del helióstato"""
    ...
    def set_axis_state(self, node_id: int, state: int):
        """Configura el estado del eje (IDLE, CLOSEDLOOP, etc.)"""
        msg_id = (node_id << 5) | SET_AXIS_STATE
        self.bus.send(self._mk_msg(msg_id, struct.pack("<I", state)))
    
    def start(self):
        """Inicia los ODrive en modo closed-loop"""
    ...
    def send_body_velocity(self, v_forward: float, omega: float):
        """Envía velocidad del cuerpo (forward m/s, rotación rad/s)"""
        # Cinemática diferencial: vR = v + omega*L/2, vL = v - omega*L/2
    ...
    def move_to_xy(self, target_x: float, target_y: float, current_x: float, 
        ...
        """
        Mueve el robot hacia una posición XY objetivo con respecto a la torre
        
        Args:
            target_x: Posición X objetivo (m)
            target_y: Posición Y objetivo (m)
            current_x: Posición X actual (m)
            current_y: Posición Y actual (m)
            current_psi: Orientación actual (radianes)
            kp_linear: Ganancia proporcional para velocidad lineal
            kp_angular: Ganancia proporcional para velocidad angular
            tolerance_m: Tolerancia para considerar que llegó (m)
        ...
        target_angle = math.atan2(dx, dy)  # En ENU: atan2(dx, dy)
        ...
        if abs(angle_error) > math.radians(30):
            v_forward *= 0.3  # Reducir velocidad lineal mientras rota
        
        self.send_body_velocity(v_forward, omega)
        return False
    
    def request_encoder_estimates(self):
        """Solicita estimaciones de encoder (posición y velocidad) de ambos motores"""
        # Solicitar de rueda derecha
        msg_id_R = (self.cfg.node_right << 5) | GET_ENCODER_ESTIMATES
        self.bus.send(self._mk_msg(msg_id_R))
        
        # Solicitar de rueda izquierda
    ...
    def read_encoder_messages(self, timeout=0.01):
        """Lee mensajes CAN de respuesta de encoders"""
        t_start = time.time()
        while (time.time() - t_start) < timeout:
            try:
                ...
                # Decodificar arbitration_id: node_id está en los bits superiores
    ...
    def get_wheel_velocities(self) -> tuple:
        """Retorna velocidades de ruedas en m/s (vR, vL)"""
        # Convertir de rev/s a m/s
    ...
    def get_encoder_positions(self) -> tuple:
        """Retorna posiciones de encoder en revoluciones (posR, posL)"""
...
class CameraDetection:
    ...
    def _detectar_marcas(self, frame_u):
        """Detecta 4 marcas VERDES del patrón"""
        # Convertir a HSV para detectar verde
        hsv = self.cv2.cvtColor(frame_u, self.cv2.COLOR_BGR2HSV)
        
        # Rango para detectar verde (ajusta según tu verde específico)
        ...
        # Encontrar contornos de las marcas verdes
        ...
        for cnt in contours:
            ...
            if 30 < area < 1500:  # Ajusta según el tamaño de tus marcas
    ...
    def get_ray_info(self) -> Dict:
        ...
        result = {
            ...
            'yaw_deg': yaw_act,
            'frame': frame_u
        }- Posición por CÁMARA+IMU (principal)
        self.helio_body_camera = Circle((0, 0), 0.25, color='green', alpha=0.7, label='Helióstato (Cámara+IMU)')
        ...
        # Helióstato - Posición por ODOMETRÍA (encoders Hall)
        self.helio_body_odom = Circle((0, 0), 0.20, color='blue', alpha=0.5, label='Odometría (Encoders)', linestyle='--', fill=False, linewidth=2)
        ...
        self.ax_map.legend(loc='upper right', fontsize=9
                self.cv2.circle(frame_u, (int(x), int(y)), 8, (0, 255, 0), -1)  # Verde brillante
                self.cv2.circle(frame_u, (int(x), int(y)), 10, (0, 200, 0), 2)   # Borde verde oscuro
            ...
            # Centro del patrón (entre las 4 marcas verdes)
            cx, cy = np.mean(pts, axis=0).astype(int)
            gray = self.cv2.cvtColor(frame_u, self.cv2.COLOR_BGR2GRAY)
            
            # Buscar el RAYO SOLAR BLANCO dentro del patrón, odom_pose: Optional[tuple] = None):
        """Actualiza la visualización con nueva información
        
        Args:
            ray_info: Información de cámara+IMU
            current_tilt: Ángulo actual de tilt
            target_tilt: Ángulo objetivo de tilt
            odom_pose: Tupla (x_odom, y_odom, psi_odom) de odometría (opcional)
        """
        # Posición CÁMARA+IMU (principal)
        x_cam, y_cam = ray_info['x_m'], ray_info['y_m']
        psi_cam = ray_info['psi']
        
        self.helio_body_camera.set_center((x_cam, y_cam))
        
        # Actualizar flecha de orientación CÁMARA
        ...
        self.helio_label_camera.set_text(f'Cám+IMU\n({x_cam:.2f}, {y_cam:.2f})')
        
        # Posición ODOMETRÍA (si está disponible)
        diff_str = ""
...
Orientación: {math.degrees(psi_cam):6.1f}°
Distancia:   {ray_info['distance_cm']:6.0f} cm{diff_str}psi_odom)
            ...
            diff_str += f"  ΔX: {diff_x:+6.2f} m\n"
            diff_str += f"  ΔY: {diff_y:+6.2f} m\n"
            diff_str += f"Dif. Ángulo:   {diff_angle:+6.1f}°"
        else:
            # Ocultar odometría si no está disponible
            ...
            # Rayo solar (blanco con borde amarillo para que resalte)
            self.cv2.circle(frame_u, bright_pt, 12, (0, 255, 255), 2)   # Borde amarillo
            self.cv2.circle(frame_u, bright_pt, 8, (255, 255, 255), -1) # Centro blanco
            
            # Línea del offset (del centro al rayo)
...
class MapVisualizer:
    ...
    def __init__(self, cfg: Cy odometría si está habilitado
        ...
        mode_str = "tilt+cámara+ODrive+Odometría
        ...
        # Panel deActualizar odometría si está habilitada
                odom_pose = None
                if self.wheel_odom:
                    # Solicitar datos de encoders
                    self.odrive.request_encoder_estimates()
                    self.odrive.read_encoder_messages()
                    
                    # Obtener velocidades y actualizar odometría
                ...
                if current_tilt is not None:
                    self.map_viz.update(ray_info, current_tilt, target_tilt_deg, odom_poseo
...
╚════════════════════════════════════════╝
 (principal)
                ...
                # Control de posición XY (usa cámara+IMU como referencia)
...
class TiltCameraController:
    def __init__(self, cfg: Config, enable_odrive: bool = False):
        ...
        # Inicializar ODrive si está habilitado
        self.enable_odrive = enable_odrive
        if enable_odrive:
            self.odrive = ODriveCanDriver(cfg, self.canm)
            self.odrive.start()
        ...
        print("\n📊 Creando ventana de visualización...")
        self.map_viz = MapVisualizer(cfg)
        mode_str = "tilt+cámara+ODrive" if enable_odrive else "tilt+cámara"
    ...
    def run_test(self, target_tilt_deg: float, duration_s: float = 120.0):
        ...
        Args:
            ...
            duration_sActualizar odometría
                    self.odrive.request_encoder_estimates()
                    self.odrive.read_encoder_messages()
                    vR, vL = self.odrive.get_wheel_velocities()
                    self.wheel_odom.update(vR, vL)
                    ...
                    # Leer posición para visualización
                    ray_info = self.camera.get_ray_info()
                    current_tilt = self.tilt.get_current_tilt()
    ...
    def move_to_position(self, target_x: float, target_y: float, target_tilt: float, 
        ...
        """
        Mueve el helióstato a una posición XY con respecto a la torre
        y ajusta el tilt del espejo
        
        Args:
            target_x: Posición X objetivo (m, positivo = Este)
            target_y: Posición Y objetivo (m, positivo = Norte)
            target_tilt: Ángulo objetivo del espejo (°)
            timeout_s: Timeout máximo (s)
            position_tolerance: Tolerancia de posición (m)
        
        Returns:
            True si llegó exitosamente, False en caso contrario
        """
        if not self.odrive:
            print("❌ ODrive no está habilitado. Inicializa con enable_odrive=True")
            return False
        
        print("=" * 80)
        print(f"🎯 MOVIMIENTO A POSICIÓN XY")
        print(f"   Objetivo: X={target_x:.2f}m, Y={target_y:.2f}m")
        print(f"   Tilt objetivo: {target_tilt:.1f}°")
        print(f"   Tolerancia: {position_tolerance}m")
        print("=" * 80)
        ...
        try:
            while (time.time() - t_start) < timeout_s:
                # Leer posición actual desde cámara/BNO055
                ray_info = self.camera.get_ray_info()
                current_x = ray_info['x_m']
                current_y = ray_info['y_m']
                current_psi = ray_info['psi']
                
                # Control de posición XY
                arrived = self.odrive.move_to_xy(
                    target_x, target_y,
                    current_x, current_y, current_psi,
                    tolerance_m=position_tolerance
                ...
                # Actualizar visualización
                if current_tilt is not None:
                    self.map_viz.update(ray_info, current_tilt, target_tilt)
    ...
    def test_odrive_movement(self, duration_s: float = 30.0):
        """
        Test simple de movimiento con ODrive
        
        Args:
            duration_s: Duración del test (s)
        """
        if not self.odrive:
            print("❌ ODrive no está habilitado")
            return
        ...
        try:
            # Secuencia de movimiento simple: adelante, girar, atrás
            phases = [
            ...
            for phase_name, v_forward, omega, phase_duration in phases:
                ...
                print(f"\n➡️ Fase: {phase_name} (v={v_forward:.2f} m/s, ω={omega:.2f} rad/s)")
                ...
                while (time.time() - phase_start) < phase_duration:
                    ...
                    if current_tilt is not None:
                        self.map_viz.update(ray_info, current_tilt, 0.0)
                    
                    print(f"\rPos: ({ray_info['x_m']:.2f}, {ray_info['y_m']:.2f})m | "
                          f"Ψ: {math.degrees(ray_info['psi']):.1f}° ",
                          end="", flush=True)
...
# ======================= MENÚ INTERACTIVO =======================
def show_menu():
    """Muestra menú interactivo y retorna las opciones elegidas"""
    print("\n" + "="*80)
    print("  🌞 SISTEMA DE CONTROL HELIÓSTATO - MENÚ PRINCIPAL")
    print("="*80)
    
    # Elegir modo de operación
    print("\n📋 MODOS DE OPERACIÓN:")
    print("  [1] Test estático    - Solo tilt y cámara (sin movimiento)")
    print("  [2] Test movimiento  - Secuencia automática con ODrive")
    print("  [3] Ir a posición XY - Mover a coordenadas específicas")
    ...
    # Mapear selección a nombre de modo
    ...
    while True:
        try:
            target_tilt = float(input("👉 Ángulo tilt del espejo (0-90°) [default: 30]: ").strip() or "30")
            if 0 <= target_tilt <= 90:
                break
    ...
    # Si modo es goto_position, pedir coordenadas
    target_x = target_y = None
    if test_mode == 'goto_position':
        print("\n📍 POSICIÓN OBJETIVO (respecto a la torre):")
        while True:
            try:
    ...
    while True:
        try:
            duration = float(input("👉 Duración del test en segundos [default: 120]: ").strip() or "120")
            if duration > 0:
                break
...
def main():
    ...
    cfg.wheel_radius = 0.08      # Radio de rueda (m)
    cfg.wheelbase = 0.30         # Distancia entre ruedas (m)
    cfg.node_right = 1           # ID nodo derecho
    cfg.node_left = 2            # ID nodo izquierdo
    cfg.max_linear_speed = 0.5   # Velocidad máxima lineal (m/s)
    cfg.max_angular_speed = 1.0  # Velocidad angular máxima (rad/s)
    cfg.brightness_threshold = 200  # Umbral brillo para detección
    # ===== FIN CONFIGURACIÓN =====
    ...
    if test_mode == "test_static":
        ...
        print("="*80)
        print(f"\n  📊 Tilt objetivo: {target_tilt:.1f}°")
        print(f"  ⏱️  Duración: {test_duration:.0f}s")
        print("\n" + "="*80 + "\n")
        ...
        print("  (Secuencia de movimientos automática)")
        print("="*80)
        print(f"\n  📊 Tilt objetivo: {target_tilt:.1f}°")
        print(f"  ⏱️  Duración: {test_duration:.0f}s")
        print("\n" + "="*80 + "\n")
        ...
        print("  🎯 MOVIMIENTO A POSICIÓN XY")
        print(f"  Objetivo: X={target_x:.2f}m, Y={target_y:.2f}m")
        print(f"  Tilt: {target_tilt:.1f}°")
        print("="*80)
        print(f"\n  📊 Coordenadas objetivo: ({target_x:.2f}, {target_y:.2f}) m")
        print(f"  📊 Tilt objetivo: {target_tilt:.1f}°")
        print(f"  ⏱️  Timeout máximo: 60s")
        print("\n" + "="*80 + "\n")
        
        input("📌 Coloca el helióstato en posición inicial y presiona ENTER para comenzar...")
        ...
        if success:
            print("\n✅ ¡Misión cumplida!")
            # Mantener posición por un tiempo
            print("\n📍 Manteniendo posición por 30s...")
            controller.run_test(target_tilt, duration_s=30.0)
        else:
            print("\n❌ No se pudo alcanzar la posición")
    
    else:
        print("❌ Modo de test no reconocido")
        print("="*80 + "\n")
